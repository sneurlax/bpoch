...
asks.unshift({price: asks[0].price, amount: 0});
...
focus.append('rect')
  .attr('id', 'crosshairBackgroundLeft')
...
g.append('rect')
  ...
  .on('mousemove', function() { 
    ...
    focus.select('#volumeTextLeft')
      .attr('x', 4)
      .attr('y', y+4)
      .text(d3.format(',.8f')(d.amount));
    focus.select('#volumeTextRight')
      .attr('x', width-46)
      .attr('y', y+4)
      .text(d3.format(',.8f')(d.amount));
    focus.select('#priceTextTop')
      .attr('x', x - (d3.format(',.2f')(d.price)).length*2.25)
      .attr('y', 4)
      .text(d3.format(',.2f')(d.price));
    focus.select('#priceTextBottom')
      .attr('x', x - (d3.format(',.2f')(d.price)).length*2.25)
      .attr('y', height-4)
      .text(d3.format(',.2f')(d.price));

    focus.select('#crosshairBackgroundLeft')
      .attr('y', y-8)
      .attr('width', (d3.format(',.8f')(d.amount)).length*5.85);
    focus.select('#crosshairBackgroundRight')
      .attr('x', width-50)
      .attr('y', y-8)
      .attr('width', 66);
    focus.select('#crosshairBackgroundTop')
      .attr('x', x - (d3.format(',.2f')(d.price)).length*2.25-4)
      .attr('width', (d3.format(',.2f')(d.price)).length*6+4);
