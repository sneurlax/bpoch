...
(function(sc) {
    ...
    sc.data.feed.coinbase.webSocket = function() {
        ...
        function webSocket(cb) {
            webSocket.close();
            coinbaseSocket = new WebSocket('wss://api.poloniex.com');
            var msg = {
                type: 'subscribe',
                'product_id': product
            };
...
(function(sc) {
    'use strict';
    sc.data.feed.poloniex.webSocket = function() {
        var product = 'BTC-USD';
        var msgType = 'match';
        var poloniexSocket = null;

        function webSocket(cb) {
            webSocket.close();
            poloniexSocket = new WebSocket('wss://ws-feed.exchange.poloniex.com');
            var msg = {
                type: 'subscribe',
                'product_id': product
            };

            poloniexSocket.onopen = function(event) {
                poloniexSocket.send(JSON.stringify(msg));
                cb(event, null);
            };

            poloniexSocket.onmessage = function(event) {
                var messageData = JSON.parse(event.data);
                if (messageData.type === msgType) {
                    var datum = {};
                    datum.date = new Date(messageData.time);
            ...
            poloniexSocket.onerror = function(event) {
                cb(event, null);
            };

            poloniexSocket.onclose = function(event) {
                cb(event, null);
            };
            ...
            var dataInterface = sc.data.dataInterface()
                .on('messageReceived', function(socketEvent, data) {
                    if (socketEvent.type === 'error' ||
                        ...
                        console.log('Error loading data from websocket: ' +
